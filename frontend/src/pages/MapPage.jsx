...
function MapboxMap({ farms, selectedId, onSelect, userLocation, radius, dark, mapStyleUrl }) {
  ...
  useEffect(() => {
    ...
    load().then(() => {
      ...
      map.on('load', () => {
        // Source pro farmy — bez clusteru, piny vždy viditelné
  ...
  useEffect(() => {
    const buildMarkers = (map) => {
      ...
      farms.forEach(farm => {
        ...
        // Piny vždy viditelné — zmenšení při oddálení
        const updateVisibility = () => {
          const zoom = map.getZoom();
          const scale = zoom < 6 ? 0.5 : zoom < 7 ? 0.7 : zoom < 8 ? 0.85 : 1;
          el.style.transform = isActive ? `scale(${scale * 1.15})` : `scale(${scale})`;
        };
